...
export default function Calculator({ data, onBack }: { data: any, onBack?: () => void }) {
  ...
  // Calculate: given EUR input, what's the ARS result?
  const calcFromEur = (eur: number) => {
    const netEur = eur - sepaFee; // SEPA fee deducted first
  ...
  // Calculate: given ARS target, what EUR is needed?
  ...
  if (editMode === 'ars') {
    // User is editing ARS → calculate EUR
    const ars = parseFloat(arsAmount) || 0;
    displayedArs = ars;
    displayedEur = calcFromArs(ars);
  } else {
    // User is editing EUR → calculate ARS
  ...
  // For the breakdown, always use the current calculation direction
  ...
  const totalCost = toDeposit + sepaFee;
  ...
  // Generate payto: URI
  const paytoUri = `payto:${binanceIBAN.replace(/\s/g, '')}?amount=${displayedEur.toFixed(2)}&message=${encodeURIComponent(sepaReference)}`;
  ...
  return (
    <div style={{ backgroundColor: '#17212b', padding: '30px', borderRadius: '24px', boxShadow: '0 10px 40px rgba(0,0,0,0.6)', border: '1px solid #1e293b', maxHeight: '85vh', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        <label style={{ display: 'block', fontSize: '13px', color: '#94a3b8', marginBottom: '8px' }}>💶 Necesitás enviar (EUR)</label>
        <input
          type="number"
          ...
          style={{
            width: '100%', padding: '16px', backgroundColor: '#0f172a', color: '#fff',
            border: editMode === 'eur' ? '2px solid #38bdf8' : '1px solid #334155',
            borderRadius: '12px', fontSize: '18px', fontWeight: 'bold',
            boxSizing: 'border-box'
          }}
      ...
      <div style={{ backgroundColor: '#0f172a', padding: '20px', borderRadius: '16px', fontSize: '14px', marginBottom: '20px', border: '1px solid #334155' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', color: '#94a3b8' }}><span>1. Depósito SEPA Binance</span><span style={{ color: '#ef4444' }}>+ {sepaFee.toFixed(2)} €</span></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', color: '#94a3b8' }}><span>2. EUR para comprar USDC ({eurUsdc.toFixed(2)})</span><span>{beforeTradeFee.toFixed(2)} €</span></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', color: '#94a3b8' }}><span>3. Fee Trading (0.1% Spot)</span><span style={{ color: '#ef4444' }}>+ {tradingFee.toFixed(4)} €</span></div>
        ...
        <div style={{ borderTop: '1px dashed #334155', paddingTop: '16px', display: 'flex', justifyContent: 'space-between', fontWeight: 'bold' }}><span style={{color: '#f8fafc'}}>Costo Final:</span><span style={{ color: '#10b981', fontSize: '18px' }}>{displayedEur.toFixed(2)} €</span></div>
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        >
          🏦 Transferir {displayedEur.toFixed(2)} € desde mi banco
